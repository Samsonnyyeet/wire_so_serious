...
def convert_to_seconds(time_str):
    if pd.isna(time_str) or time_str == 'No recent events':
        return 0
    try:
...
df['Pod Event Age (s)'] = df['Pod Event Age'].apply(convert_to_seconds)
df['Event Age (s)'] = df['Event Age'].apply(convert_to_seconds)


df = df.rename(columns={'Active Memory Limits (MB)emory Limits Utilization (%)': 'Active Memory Limits (MB)'})


df['Target'] = (
    (df['Pod Status'].isin(['CrashLoopBackOff', 'Unknown'])) |  
    (df['Ready Containers'] < df['Total Containers']) |         
    (df['Pod Restarts'] > 0) |                                 
    ((df['Pod Event Type'] == 'Warning') & (df['Pod Event Reason'] == 'BackOff')) |  
    (df['Event Reason'] == 'OOMKilling')                       
).astype(int)
...
features = ['Network Receive Bytes', 'Network Transmit Bytes', 'Network Receive Packets (p/s)', 
            ...
            'FS Reads/Writes Bytes Total (MB)', 'CPU Usage (%)', 'Memory Usage (%)', 'Memory Usage (Cache) (MB)', 'Memory Usage (Swap) (MB)', 
            'Active Memory Requests (MB)', 'Memory Requests (%)', 
            'Pod Restarts', 'Ready Containers', 'Total Containers', 'Pod Event Age (s)', 'Event Age (s)', 
            'time_since_last_record', 'day', 'hour', 'minute']
cat_features = ['Pod Status', 'Pod Event Type', 'Pod Event Reason', 'Pod Event Source', 
                'Node Name', 'Event Reason', 'Event Source']
...
with torch.no_grad():
    ...
    auc = roc_auc_score(y_test.cpu(), outputs.squeeze().cpu())
    print(f'Test Accuracy: {accuracy.item():.4f}')
    print(f'Precision: {precision:.4f}, Recall: {recall:.4f}, F1-Score: {f1:.4f}, AUC-ROC: {auc:.4f}')

torch.save(model.state_dict(), 'service_disruptions_model.pth')